...
class Mage_Catalog_Model_Product_Attribute_Backend_Media extends Mage_Eav_Model_Entity_Attribute_Backend_Abstract
{
    ...
    public function beforeSave($object)
    {
        ...
        foreach ($value['values'] as $mediaAttrCode => $attrData) {
            if (array_key_exists($attrData, $newImages)) {
                $object->setData($mediaAttrCode . '_label', ($newImages[$attrData]['label'] === null || $newImages[$attrData]['label_use_default']) ? $newImages[$attrData]['label'] : $newImages[$attrData]['label_default']);
            }

            if (array_key_exists($attrData, $existImages)) {
                $object->setData($mediaAttrCode . '_label', ($existImages[$attrData]['label'] === null || $existImages[$attrData]['label_use_default']) ? $existImages[$attrData]['label'] : $existImages[$attrData]['label_default']);
            }
        }
